            ...
            for(int xx = 0; xx < w; xx++) {
                for(int yy = 0; yy < h; yy++) {
                    ...
                    if(red == 00 && green == 255 && blue == 0) {
                        oHandler.addObject(new Worm(xx*32, yy*32, ID.WormEnemy, oHandler));

                        if((hud.getWave() + 2) % 3 == 0) {
                            if(randNum.nextInt(2) == 0) {oHandler.addObject(new EnemySpawnerPortal(xx*32, yy*32, ID.FlyingEyeEnemy, oHandler));}
                            else {oHandler.addObject(new Worm(xx*32, yy*32, ID.WormEnemy, oHandler));}
                        }
                        else if(hud.getWave() % 2 == 0) {
                            oHandler.addObject(new arachnidMage(xx * 32, yy * 32, ID.ArachnidMageEnemy, oHandler));
                        }
